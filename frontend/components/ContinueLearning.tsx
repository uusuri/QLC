...
export function ContinueLearningCard({ nextLesson }: ContinueLearningCardProps) {
  ...
  return <section className="kit-continue">
    ...
    <div className="kit-continue-actions"><Progress label={`Решено ${lesson.solvedTasks} / ${lesson.totalTasks} задач`} value={lesson.totalTasks > 0 ? lesson.solvedTasks / lesson.totalTasks * 100 : null} /><ButtonLink href={`/lessons/${lesson.id}`}>Продолжить →</ButtonLink></div>
  </section>;
}
