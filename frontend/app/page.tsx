...
// Данные курсов приходят через сервисный слой, даже пока это мок.
import { getCourseCatalog } from "@/services/api";

// Главная страница сайта.
export default async function HomePage() {
  // Получаем каталог через services/api.ts, чтобы компоненты не знали источник данных.
      ...
      <div className="mx-auto max-w-7xl border border-line bg-ink/90">
        ...
        <section className="px-5 py-10 sm:px-7 lg:py-14" id="courses">
          ...
          <div className="grid gap-px border border-line bg-line lg:grid-cols-3">
            {/* map превращает массив courses в набор карточек. */}
            {courses.map((course) => (
              // Карточка одного курса.
              <article className="group grid bg-ink" key={course.title}>
